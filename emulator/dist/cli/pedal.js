...
import { readRollFile, rollPorts } from "../roll/ports.js";
import { halfPedalling, pedalDefaults, pedalSpans, runPedals, tiedToRise } from "../model/pedal.js";
import { pedalMessages } from "../midi/pedal.js";
import { noteOff, noteOn, setTempo, trackName, writeSmf } from "../midi/write.js";
const NOTE_TRACKS = [1, 2];
const FLAT_VELOCITY = 64;
/** Travel budgets for `--sweep`, spanning what the sources leave open. */
const BUDGETS_MS = [100, 140, 180, 220, 260, 300, 360];
...
function parameters() {
    ...
    const given = Object.fromEntries(Object.entries(pedalDefaults).map(([name, value]) => [name, Number(option(dashed(name), String(value)))]));
    return flag("tied") ? tiedToRise(given) : given;
}
function noteMessages(roll) {
    return NOTE_TRACKS.flatMap((index, channel) => (roll.smf.tracks[index] ?? []).flatMap((event) => {
        if (event.kind === "note-on")
            return [noteOn(event.tick, channel, event.key, FLAT_VELOCITY)];
...
function tempoTrack(roll, name) {
    return [
        trackName(name),
        ...roll.timing.segments.map((segment) => setTempo(segment.tick, Math.round((roll.smf.division * 1e6) / segment.ticksPerSecond))),
    ];
}
/**
 ...
 */
function travelCsv(travel, grid) {
    const level = (value) => Math.round(value * 127);
...
function sweep(input) {
    console.table(BUDGETS_MS.map((total) => {
        const relayLagMs = Math.round(total / 3);
        const bellowsMs = total - relayLagMs;
        const spans = pedalSpans(runPedals(input, { ...pedalDefaults, relayLagMs, liftMs: bellowsMs, fallMs: bellowsMs }), input.grid);
...
function main() {
    const mode = option("mode", "continuous");
    ...
    console.log(`damper: ${summary.presses} presses and ${summary.lifts} lifts asked for; ` +
        `${summary.unfinishedPresses} presses and ${summary.unfinishedLifts} lifts cut short`);
    if (summary.unfinishedLifts > 0) {
        console.log(`        the deepest unfinished lift left the dampers ` +
    ...
    console.table(lifts.slice(0, 8).map((span) => ({
    ...
    const ticks = (index) => Math.max(grid.rowAt(index) - roll.timing.firstHole, 0);
