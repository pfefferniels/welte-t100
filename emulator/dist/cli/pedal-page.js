...
import { DAMPER_CC, SOFT_CC } from "../midi/pedal.js";
...
function transit(series, grid, from = -Infinity, to = Infinity) {
    let moving = 0;
    let total = 0;
    series.forEach((value, index) => {
...
function perforationMarks(punches) {
    return punches
...
function page(from, travel, params, piece, label) {
    ...
    const byLength = (a, b) => a.milliseconds - b.milliseconds;
    const median = (of) => of.length === 0 ? 0 : of.toSorted(byLength)[Math.floor(of.length / 2)].milliseconds;
    ...
    const railWindow = railEdges.length > 1
    ...
    const railFigure = railEdges.length > 1
        ? `<figure>
