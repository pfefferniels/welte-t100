...
import { agreement, bestLag } from "../eval/metrics.js";
...
function fixed(value, places = 4) {
    return Number.isFinite(value) ? value.toFixed(places) : "—";
}
function main() {
    const druid = process.argv[2] ?? "jq774vx6544";
    const loaded = loadRoll(druid);
    const secondsPerRow = (loaded.grid.seconds.at(-1) - loaded.grid.seconds[0]) / (loaded.grid.length - 1);
    ...
    const rows = MODELS.flatMap(({ model, ports }) => HALVES.map((half) => {
