...
import { agreement, intersect } from "../eval/metrics.js";
import { alternatingBlocks } from "../eval/split.js";
import { pneumaticModel } from "../model/pneumatic.js";
import { REGIMES, regimesOf } from "../model/field.js";
import { latched, portOf } from "../model/latch.js";
import { describeTraversals } from "../model/timings.js";
function option(name, fallback) {
    const at = process.argv.indexOf(`--${name}`);
    return at >= 0 ? (process.argv[at + 1] ?? fallback) : fallback;
}
function worstWindows(error, mask, seconds, windowSeconds, count) {
...
function main() {
    ...
    fit.results.forEach(({ half, params }) => {
        ...
        console.log(`\n=== ${half} ===`);
        console.table(REGIMES.map((name, code) => {
        ...
        console.table(Array.from({ length: 5 }, (_, band) => {
            const low = band / 5;
            const high = (band + 1) / 5;
            const mask = intersect(test, Uint8Array.from(truth.value, (value) => (value >= low && value < high ? 1 : 0)));
            const scored = agreement(model, truth.value, mask);
            return {
        ...
        test.forEach((observed, index) => {
            if (observed)
                totalSquare += error[index] * error[index];
        });
        console.log("error by how fast the drawn line is moving");
        console.table(bands.slice(0, -1).map((low, band) => {
            const high = bands[band + 1];
            const mask = intersect(test, Uint8Array.from(speed, (value) => (value >= low && value < high ? 1 : 0)));
            const scored = agreement(model, truth.value, mask);
            let share = 0;
            mask.forEach((observed, index) => {
                if (observed)
                    share += error[index] * error[index];
            });
            return {
