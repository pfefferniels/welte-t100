 ...
 */
export function contiguousSplit(grid, observed, trainFraction = 0.6) {
...
export function alternatingBlocks(grid, observed, blockSeconds = 8) {
    const origin = grid.seconds[0];
    const isTrain = Float64Array.from(grid.seconds, (second) => Math.floor((second - origin) / blockSeconds) % 2 === 0 ? 1 : 0);
    return {
