 ...
 * schwächer als Mezzoforte spielen kann" (Reinhard Hagemann, "Einstellanleitung
 * für Welte-Mignon", Das Mechanische Musikinstrument 80, 2001, p. 26). A
 * floor-only reading was carried as an option and priced on roll 3309; it never
 * differed, because the line all but never meets the hook from below. Across the
 * six lined rolls 84 of the 86 witnessed engagements find the line above the
 * face, the two exceptions on 1474.
 *
 * The consequence for reading a roll is that a measured stop level is the rest
 * the bellows happened to arrive at, not the pin's centre. On every lined roll
 * the engagements happen with the line falling from the forte rail, so what is
 * measured is the falling rest and the centre lies half a thickness above it.
 */
/**
 * Only the pin's lower face is observable: a census of the six lined rolls finds
 * 84 of 86 witnessed engagements entered from above, at the fortissimo rail, and
 * on roll 3309 the fastest approach from below is 3.8 units/s against 14 to 16
 * at a real arrest. Fitting the
 ...
 */
export const MF_THICKNESS = 0.06;
export function newStopState() {
    return { engaged: false, trappedAbove: false };
}
/**
 ...
 */
export function limitAtStop(state, engaged, current, moved, level, thickness) {
    if (!engaged) {
        state.engaged = false;
        return moved;
    }
    ...
    return state.trappedAbove ? Math.max(moved, level - thickness / 2) : Math.min(moved, level + thickness / 2);
}
