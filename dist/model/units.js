...
const LEVELS = ["mezzoforte", "crescendoTarget", "releaseTarget", "sforzandoTarget"];
const CONDUCTANCES = ["crescendoRate", "releaseRate", "sforzandoRate", "sforzandoAssistRate"];
function rescaled(params, piano, forte, toTravel) {
    ...
    LEVELS.forEach((name) => {
        if (name in out)
            out[name] = level(out[name]);
    });
    CONDUCTANCES.forEach((name) => {
        if (name in out)
            out[name] = conductance(out[name]);
    });
    if ("throughFlowLoad" in out)
        out.throughFlowLoad = load(out.throughFlowLoad);
...
export function inTravelUnits(params) {
...
export function onPrintedScale(instrument, piano, forte) {
