 ...
 */
import { drive } from "../core/flow.js";
import { heldAbove, heldValve } from "../core/valve.js";
import { shiftedPortOf } from "./codes.js";
/**
 ...
 */
const WHOLE_BAND = 1;
export function t98Relay(input, params) {
    const p = params;
    const alpha = p.alpha;
    ...
    const at = (control, extra) => shiftedPortOf(input, control, lead + extra, drift);
    const valve = (spec, control, extra) => heldValve(at(control, extra), input.grid.dt, spec);
    const sforzandoPiano = valve({ fillMs: p.pianoFillMs, tailMs: p.pianoTailMs, tripThreshold: trip, band: p.pianoBand }, "sforzandoPiano", 0);
    const sforzandoForte = valve({ fillMs: p.forteFillMs, tailMs: p.forteTailMs, tripThreshold: trip, band: p.forteBand }, "sforzandoForte", p.leadSforzandoOnRows);
    const crescendo = valve({ fillMs: p.crescendoFillMs, tailMs: p.crescendoTailMs, tripThreshold: trip, band: WHOLE_BAND }, "crescendo", p.leadCrescendoRows);
    ...
    const mezzoforte = valve({ fillMs: p.mezzoforteFillMs, tailMs: p.mezzoforteTailMs, tripThreshold: trip, band: WHOLE_BAND }, "mezzoforte", p.leadMezzoforteRows);
    ...
    return {
        ...
        drive: (state, index) => {
            ...
            const throughFlow = throughFlowLoad > 0 && forteRate > 0 ? (admitted * drawnRate * towardsOpen) / forteRate : 0;
            const supply = 1 / (1 + throughFlowLoad * throughFlow);
            const sag = (target) => releaseTarget + (target - releaseTarget) * supply;
