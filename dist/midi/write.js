...
function varInt(value) {
    const septets = (rest) => (rest === 0 ? [] : [...septets(Math.floor(rest / 128)), rest % 128]);
...
function metaMessage(tick, type, data) {
    return { tick, bytes: [META, type, ...varInt(data.length), ...data] };
}
export function noteOn(tick, channel, key, velocity) {
    return { tick, bytes: [NOTE_ON | channel, key, velocity] };
}
export function noteOff(tick, channel, key) {
    return { tick, bytes: [NOTE_OFF | channel, key, 0] };
}
export function controlChange(tick, channel, controller, value) {
    return { tick, bytes: [CONTROL_CHANGE | channel, controller, value] };
}
export function setTempo(tick, microsPerQuarter) {
...
function trackChunk(track) {
    ...
    if (first && first.tick < 0)
        throw new RangeError(`message at tick ${first.tick}: ticks start at zero`);
    const end = metaMessage(ordered.at(-1)?.tick ?? 0, END_OF_TRACK, []);
    const messages = [...ordered, end];
    return chunk("MTrk", messages.flatMap((message, index) => [
        ...varInt(message.tick - (messages[index - 1]?.tick ?? 0)),
        ...message.bytes,
    ]));
}
export function writeSmf(division, tracks) {
