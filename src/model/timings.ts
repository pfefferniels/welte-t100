...
export type Traversal = { readonly from: number; readonly to: number; readonly milliseconds: number };

const STEP_SECONDS = 1e-4;
const LIMIT_SECONDS = 30;

function travelTime(from: number, to: number, rate: number, target: number, alpha: number): number {
  ...
  while (elapsed < LIMIT_SECONDS && Math.sign(to - x) === direction) {
    ...
    if (Math.sign(speed) !== direction || speed === 0) return Number.POSITIVE_INFINITY;
 ...
 */
export function traversals(params: Parameters): Record<string, Traversal> {
  const p = params as Record<string, number>;
  const piano = p.piano ?? 0;
  const forte = p.forte ?? 1;
  const mezzoforte = p.mezzoforte ?? 0.5;
  const alpha = p.alpha ?? 1;
  const at = (from: number, to: number, rate: number, target: number): Traversal => ({
    from,
    to,
...
export function describeTraversals(params: Parameters): { span: string; from: string; to: string; ms: string }[] {
