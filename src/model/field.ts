...
export const REGIMES = ["quiescent", "crescendo", "sforzando", "release"] as const;
...
export function regimesOf(input: ModelInput): Uint8Array {
  ...
  return Uint8Array.from(crescendo, (isCrescendo, index) =>
    releasing[index] ? 3 : sforzando[index] ? 2 : isCrescendo ? 1 : 0,
  );
}

export function estimateField(input: ModelInput, truth: TracedCurve, mask: Mask): VelocityField {
  ...
  for (let index = width; index + width < truth.value.length; index += 1) {
    if (!mask[index] || !truth.observed[index - width] || !truth.observed[index + width]) continue;
 ...
 */
function filled(field: VelocityField): Float64Array {
  const table = Float64Array.from(field.mean);
  REGIMES.forEach((_, regime) => {
    const populated = Array.from({ length: BINS }, (_, bin) => bin).filter(
      (bin) => field.count[regime * BINS + bin]! > 0,
    );
    if (populated.length === 0) return;
    Array.from({ length: BINS }, (_, bin) => bin)
      .filter((bin) => field.count[regime * BINS + bin] === 0)
      .forEach((bin) => {
        const nearest = populated.reduce((best, other) =>
          Math.abs(other - bin) < Math.abs(best - bin) ? other : best,
        );
...
function speedAt(table: Float64Array, regime: number, x: number): number {
...
type State = { x: number; velocity: number; stop: StopState };

export function fieldModel(field: VelocityField): Model {
  const table = filled(field);

  const run = (input: ModelInput, params: Parameters): Float64Array => {
