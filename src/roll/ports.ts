...
import { aperturePorts, DEFAULT_GEOMETRY, type PortGeometry } from "./aperture.ts";
import { perforations, type Perforation } from "./expression.ts";
import { Grid } from "./grid.ts";
import { pixels, readRoll, type Roll } from "./timing.ts";

export function readRollFile(path: string, druid = path): Roll {
 ...
 */
export function geometryOf(roll: Roll): PortGeometry {
 ...
 * it changes nothing.
 */
export function bridged(punches: readonly Perforation[], geometry: PortGeometry): Perforation[] {
  ...
  return [...byPort.values()]
    .flatMap((port) =>
      port
        ...
        .reduce<Perforation[]>((joined, punch) => {
          const last = joined.at(-1);
          if (!last || punch.rowOn - last.rowOff >= bore) return [...joined, punch];
          const grown = {
            ...last,
...
export type RollPorts = {
  readonly roll: Roll;
  readonly grid: Grid;
  readonly ports: ReturnType<typeof aperturePorts>;
 ...
 */
export function rollPorts(roll: Roll): RollPorts {
